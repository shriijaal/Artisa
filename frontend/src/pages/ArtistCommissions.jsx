...
const ArtistCommissions = () => {
  ...
  const urgentCount = commissions.filter(c => {
        ...
        <div className="flex items-start justify-between mb-8 flex-wrap gap-4">
          <div>
            ...
            <p className="text-sm text-stone-500 mt-1">
              {commissions.length} total · {commissions.filter(c => ['pending', 'accepted', 'in_progress'].includes(c.status)).length} active
            </p>
          </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-3">
            {filtered.map((c) => {
              ...
              const isNewRequest = c.status === 'pending';
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        {isNewRequest && (
                          <span className="flex-shrink-0 rounded-full bg-yellow-100 text-yellow-700 text-xs font-bold px-2 py-0.5 border border-yellow-200">
                            New
                          </span>
                        )}
                      ...
                      <p className="text-sm text-stone-500">
                        From: <span className="font-medium">{c.customer?.username}</span>
                      </p>
